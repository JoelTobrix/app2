...
class question3movil : AppCompatActivity() {
    @SuppressLint("MissingInflatedId", "SuspiciousIndentation")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.question3movil)
        ...
        button.setOnClickListener {
            var seleccionar = ""
            if (radi1.isChecked) {
                seleccionar += " Incorrecto " + " Tiene 0 "
            }
            if (radi2.isChecked) {
                seleccionar += " Correcto " + " Tiene 10 "
            }
            if (radi3.isChecked) {
                seleccionar += " Incorrecto " + " Tiene 0 "
            }
            if (seleccionar.isEmpty()) {
                ...
                Toast.makeText(applicationContext, "Selección: $seleccionar", Toast.LENGTH_SHORT)
                    .show()
